...
盯住一个 GitHub PR（pull request，一次待合并的代码改动）上的 Codex AI review，边跑边修每一条发现，并在合适的时机停下——要么 Codex 给出 👍，要么你判断剩下的问题已经都不重要了——而不是无止境地追着 nitpick（那种不改也没事的吹毛求疵意见）跑。
...
- **自愈式 monitor** —— 每个轮询周期只发一条 GraphQL 查询，同时盯住 review、行内发现、PR 上的 issue 评论、👍 reaction 和 CI（自动跑的构建与测试）。如果 head commit（分支上最新的那个提交）已经 CI 全绿（自动检查全部通过）但 Codex 还没有任何反应，monitor 会对每个没被 review 过的 head 自动发一条 `@codex review` —— 不需要人去发现这个卡点。
...
- **停止的决定权在你手里，不在 bot 手里。** 三个合法的退出条件，任意一个满足就可以结束循环：
  1. Codex 给出 👍 —— 最干净的方式。
  2. 你判断已经到了**严重度下限**，也就是「剩下的问题都轻到可以收工」这条底线：没有未解决的 P0、你这次改动碰到的代码路径上没有未解决的 P1、也没有 regression（本来好好的功能被这次改动弄坏了）—— 剩下的都归档成 follow-up issue（留到以后再处理的待办条目）。
  3. **轮次上限**：循环最多跑几轮的硬性上限，作为兜底，防止 Codex 无限重复提同样的 nitpick。

  <p align="center">
  <img src="docs/images/03-three-exits.png" alt="停止的决定权在人手里，三扇门是三个合法的退出条件" width="640">
  </p>

- **P0–P3 决策表**：Codex 会给每条意见打一个严重度标签，决策表据此决定哪些在这个 PR 里直接修，哪些归档成 follow-up。

  | 标签 | 含义 |
  | --- | --- |
  | **P0** | 真的会出事：崩溃、数据算错、安全漏洞。必须在本 PR 里修。 |
  | **P1** | 确实是问题，但没那么急。落在你这次改动路径上就修，否则记成待办。 |
  | **P2** | 建议性的改进意见，默认归档。 |
  | **P3** | 风格、命名之类的 nitpick，默认归档。 |
